...
static std::vector<size_t> makeJacobsthal(size_t n) {
...
std::vector<size_t> PmergeMe::buildInsertionOrder(size_t pendSize) {
	std::vector<size_t> jac = makeJacobsthal(pendSize);
...
std::vector<std::pair<int, int> > PmergeMe::makePairsVec(
		std::vector<int>& arr, size_t pairCount) {
...
std::deque<std::pair<int, int> > PmergeMe::makePairsDeq(
		std::deque<int>& arr, size_t pairCount) {
...
std::vector<std::pair<int, int> > PmergeMe::reorderPairsVec(
...
std::deque<std::pair<int, int> > PmergeMe::reorderPairsDeq(
...
void PmergeMe::insertPendVec(std::vector<int>& chain,
...
void PmergeMe::insertPendDeq(std::deque<int>& chain,
...
void PmergeMe::mergeInsertVec(std::vector<int>& arr) {
	...
	std::vector<std::pair<int, int> > pairs = makePairsVec(arr, pairCount);
	...
	pairs = reorderPairsVec(pairs, largers);
	...
	if (!pend.empty())
		insertPendVec(chain, pend, pairs);
...
void PmergeMe::mergeInsertDeq(std::deque<int>& arr) {
	...
	std::deque<std::pair<int, int> > pairs = makePairsDeq(arr, pairCount);
	...
	pairs = reorderPairsDeq(pairs, largers);
	...
	if (!pend.empty())
		insertPendDeq(chain, pend, pairs);
