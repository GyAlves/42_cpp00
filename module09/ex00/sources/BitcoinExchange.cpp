...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/04/15 19:16:49 by galves-a          #+#    #+#             */
/*   Updated: 2026/04/15 19:51:05 by galves-a         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void BitcoinExchange::processInput(std::string const& filename) const {
    ...
    while (std::getline(file, line)) {
        ...
        printExchangeValue(date, value);
    }
}

bool BitcoinExchange::parseInputLine(std::string const& line, std::string& date, float& value) const {
    ...
    if (!isValidDate(date) || !isValidValueFormat(valueStr)) {
        std::cout << "Error: bad input => " << line << std::endl;
        return false;
    }
...
void BitcoinExchange::printExchangeValue(std::string const& date, float value) const {
    try {
...
bool BitcoinExchange::isValidValueFormat(std::string const& valueStr) const {
    size_t start = 0;
    bool hasDot = false;
